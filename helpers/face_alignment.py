...
class LandmarkDetector:
    ...
    def get_landmarks(self, img):
        ...
        img = img.unsqueeze(0)
        landmarks = self.model(img)
        landmarks = landmarks.detach().numpy()
        landmarks = landmarks.reshape(-1, 2)
        return landmarks
    
    def angle_detection(self, img):
        landmarks = self.get_landmarks(img)
        # angle between jawline and nose
        jawline = landmarks[0:17]
        nose = landmarks[27:35]
        jawline = np.mean(jawline, axis=0)
        nose = np.mean(nose, axis=0)
        angle = np.arctan2(jawline[1] - nose[1], jawline[0] - nose[0])
    ...
    def align(self, img):
        angle = self.angle_detection(img)
        aligned = self.rotate(img, angle)
        return aligned

# how to download ScriptModule torchscript model
